...
\textbf{Objetivo:} consolidar la evolución de TiendaTech y evaluar qué propiedades de su arquitectura distribuida quedan respaldadas por evidencia. \textbf{Método:} revisión del repositorio, mediciones de CockroachDB y Spark, y una campaña de 120 corridas mediante API Gateway, microservicios y CockroachDB, con dos estrategias, cuatro concurrencias y tres condiciones de fallo. \textbf{Resultados:} la matriz real contiene 76022 solicitudes y alcanzó la concurrencia objetivo en sus 24 condiciones; 398 de 401 intentos de checkout fallaron y solo tres fueron confirmados. El resultado acredita ejecución y saturación reales, pero no permite comparar con solidez 2PC y Saga ni atribuir conservación de invariantes. La evaluación ISO observó 3588/3588 sondeos exitosos durante una hora y un p95 de 610\,ms con 50 usuarios, superior al objetivo de 500\,ms. El laboratorio SQLite se conserva únicamente como piloto local. \textbf{Contribución:} una memoria trazable que separa implementación, medición y límites de inferencia. No se acredita una comparación reglas--RAG.

\textbf{Palabras clave:} sistemas distribuidos, coordinación, CockroachDB, Spark, calidad, reproducibilidad.
\clearpage
\section*{Abstract}
\addcontentsline{toc}{section}{Abstract}
\textbf{Objective:} to consolidate TiendaTech's evolution and determine which distributed-system properties are supported by evidence. \textbf{Method:} repository review, CockroachDB and Spark measurements, and 120 runs through the API Gateway, microservices and CockroachDB, covering two strategies, four concurrency levels and three fault conditions. \textbf{Results:} the real matrix contains 76,022 requests and reached its target concurrency in all 24 conditions; 398 of 401 checkout attempts failed and only three were confirmed. This establishes real execution and saturation, but is insufficient for a robust 2PC versus Saga comparison or invariant claim. The SQLite laboratory is retained only as a local pilot. \textbf{Contribution:} a traceable report separating implementation, measurement and inference limits. One-hour availability and a rules-versus-RAG comparison are not established.
...
El banco SQLite histórico mostró p95 menores para E-2PC que para E-Saga, pero ese resultado pertenece a implementaciones locales y no permite concluir que el protocolo 2PC sea generalmente superior. La versión que produjo esas cifras estaba serializada por un candado global de Python; el piloto posterior retiró ese candado, mantuvo la restricción de escritor de SQLite y detectó una discordancia de stock en E-Saga. La campaña desplegada sí separa procesos y utiliza CockroachDB, pero su saturación deja solo tres checkouts confirmados y tampoco permite comparar latencias entre estrategias. Los dos estudios responden preguntas parciales distintas y no se combinan estadísticamente.

El cero histórico de violaciones procede del oráculo de la ejecución serializada y no se extiende al piloto corregido ni a la campaña desplegada. En esta última, los participantes y fallos son reales dentro del entorno de prueba, pero el CSV agregado no conserva el historial de estados necesario para evaluar cada invariante y la mayoría de operaciones no alcanza una confirmación. Se requiere fortalecer la observación con identificadores únicos, seguimiento persistente de todos los casos emitidos y controles positivos que demuestren que el oráculo detecta cada violación. La política CAP por agregado sigue siendo una decisión de diseño: ninguno de los estudios ensaya una partición de red controlada con suficiente finalización de compras para validarla.
...
La descripción histórica del candado se refiere al banco SQLite original. El piloto posterior de la sección \ref{sec:actualizacion-evidencias} elimina el candado de Python y detecta una discordancia de stock. La evaluación distribuida ya fue ejecutada en 120 corridas contra microservicios y CockroachDB; su límite vigente no es el candado local, sino la saturación observada y la escasez de checkouts confirmados.
...
\textbf{Candado global y serialización.} La versión vigente de \path{experiments/paso7/coordination_lab.py} ya no crea \texttt{self.db\_lock}: el candado global de Python fue retirado. El piloto conserva la restricción de escritor único de SQLite mediante \texttt{BEGIN IMMEDIATE}; por ello queda acotado explícitamente a demostración local y no se usa para sustentar C2, C3 o C6. La evidencia principal es la campaña de \path{experiments/paso8/resultados-reales/oficial-v4-20260904/}, ejecutada mediante Gateway, Pedidos, Ventas, Inventario y CockroachDB. Esta mitigación elimina la dependencia causal del candado local, pero la campaña real registró saturación extrema y solo tres checkouts confirmados, de modo que tampoco autoriza una conclusión comparativa fuerte entre estrategias.
...
C2, C3, C6 & Medición distribuida ejecutada: 120 corridas reales, 24 condiciones y cinco repeticiones. Análisis interpretativo pendiente por baja cantidad de compras confirmadas. & La campaña usa participantes reales y CockroachDB, pero la saturación (3 confirmaciones de 401 intentos de checkout) impide comparar estrategias con potencia suficiente. & Evidencia en \path{experiments/paso8/resultados-reales/oficial-v4-20260904/}; repetir con carga inicial menor y escalado progresivo antes de afirmar invariantes o superioridad.\\
\bottomrule\end{longtable}}
...
C7, P3 y C9 se retiran de la tabla de deuda vigente porque ya cuentan con evidencia versionada: cobertura y CI rojo/verde, panel y trazas bajo carga, arranque desde volúmenes limpios y cinco referencias específicas de coordinación. Retirarlos no cambia la valoración histórica del docente ni implica aprobación posterior; indica que la condición documental de cierre ya puede auditarse en \path{docs/evidencias/}, \path{docs/experimentos/resultados/iso25010.csv}, \path{docs/evidencias/arranque-limpio-paso15.md} y la sección \ref{sec:literatura-coordinacion}. C2, C3 y C6 permanecen porque la campaña desplegada confirmó solo tres checkouts y no sostiene la comparación o los invariantes buscados.
...
\textbf{Pregunta 1: invariantes.} La respuesta es negativa. La campaña desplegada alcanzó sus 24 condiciones y 120 corridas, pero solo 3 de 401 intentos de checkout fueron confirmados; además, su CSV registra respuestas y estados agregados, no un historial persistente suficiente para comprobar pago exacto, descuento único, stock no negativo y compensación completa por operación. La evidencia acredita concurrencia y saturación reales, pero no una muestra de compras terminadas que permita afirmar conservación de invariantes o validar la política CAP por agregado. El resultado del piloto SQLite tampoco cubre esa brecha: el oráculo histórico se ejecutó bajo serialización artificial y el piloto posterior detectó una discordancia de stock en E-Saga local.

\textbf{Pregunta 2: rendimiento.} La campaña desplegada no permite una comparación válida de latencia de compra entre estrategias: la mediana de checkouts confirmados fue cero en cada condición y los p95/p99 de 60\,000 ms observados mayoritariamente representan el límite de espera del cliente. Comparar esos límites como si fueran tiempos de procesamiento atribuiría a 2PC o Saga la saturación anterior al checkout. El resultado histórico del piloto, donde E-2PC tuvo p95 menores en las doce comparaciones, se conserva como antecedente local sin valor de generalización: usó SQLite, semillas no perfectamente emparejadas y cinco repeticiones, cuyo mínimo p bilateral exacto 0,007937 no supera el umbral de Bonferroni 0,004167.

\textbf{Pregunta 3: recuperación.} La campaña desplegada ejercitó omisión y temporización, pero sus 398 fallos entre 401 intentos de checkout no se acompañan de historiales persistentes por operación que demuestren rollback, compensación o reanudación durable. Por ello no permite atribuir recuperación correcta a ninguna estrategia. El piloto local sí modela rollback y compensación ante una excepción de autorización, pero no prueba respuesta perdida después de un pago externo exitoso, caída del coordinador ni recuperación distribuida; su tiempo de convergencia se mide desde el inicio de la compra, no como tiempo aislado de reparación.
...
\textbf{Pregunta 5: reproducibilidad distribuida.} La campaña real conserva 120 filas únicas, las 24 condiciones con cinco repeticiones, 60 segundos de calentamiento, 90 segundos de medición y concurrencias efectivamente alcanzadas. Se ejecutó a través del Gateway y los microservicios sobre CockroachDB. La validación estructural está en \path{experiments/paso8/resultados-reales/oficial-v4-20260904/analisis/validacion.json}; la desviación frente a los cinco minutos sugeridos y las reanudaciones están declaradas. Esto permite auditar la estructura y repetir el procedimiento, pero no convierte el resultado en una comparación concluyente: únicamente tres checkouts fueron confirmados.

\subsection{Balance acumulativo}
TiendaTech pasó de la propuesta arquitectónica a una implementación con clientes, servicios, datos distribuidos e instrumentos de prueba. Se documentaron decisiones de fragmentación, capas y autenticación; se incorporaron CI, cobertura, métricas, Pact, recorridos E2E y evidencia de revisión. La disponibilidad, la carga oficial, el panel y las trazas ya cuentan con evidencia fechada. El avance es verificable, pero no elimina las incidencias abiertas ni permite convertir una sola corrida de carga o tres checkouts confirmados en una comparación general de estrategias.
...
Para reproducir el PDF desde un clon se versionan el logo \path{UteqLogo.png}, las imágenes de \path{img/} y las figuras \path{cierre/boxplot-p95.png} y \path{cierre/spark-t1.png}. La evidencia distribuida principal conserva las 120 corridas de microservicios y CockroachDB, su resumen y validación en \path{experiments/paso8/resultados-reales/oficial-v4-20260904/}. El banco SQLite de \path{experiments/paso7/coordination_lab.py} permanece como piloto local histórico y no sustenta C2, C3 ni C6. \path{CITATION.cff} registra autoría y citación.
