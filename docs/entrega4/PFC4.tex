...
La tabla \ref{tab:cap} (``Consistencia por agregado'') formula una política CAP distinta para inventario, pedido/pago, usuarios, catálogo y analítica, cada una con su límite de evidencia declarado. \textbf{Pendiente honesto:} sostener esta posición con los datos del Paso 8 requiere que \texttt{experiments/paso8} se reejecute con las correcciones de concurrencia ya integradas en \texttt{main} (commits \texttt{5905639} y \texttt{5cef707}); el \texttt{experimento\_crudo.csv} vigente sigue fechado antes de esas correcciones, así que este ítem no se cierra todavía con datos definitivos.\\
