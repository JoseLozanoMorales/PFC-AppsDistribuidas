package com.example.productos.presentation;

import com.example.productos.presentation.dto.ProductoCreadoResponse;
import com.example.productos.presentation.dto.CrearProductoRequest;
import com.example.productos.presentation.dto.ProductoResumenResponse;
...
public class ProductoController {
    ...
    public List<ProductoResumenResponse> listar(
        ...
        return service.listar(page, size).stream().map(ProductoController::toResponse).toList();
    }
    ...
    public ResponseEntity<Map<String, Object>> agregarImagen(@PathVariable Integer id,
        ...
        ImagenProducto imagen = new ImagenProducto(file.getBytes(), file.getContentType(), file.getSize());
        Long galeriaId = service.agregarImagen(id, imagen, descripcion, portada);
    ...
    public ResponseEntity<ByteArrayResource> galeriaContenido(@PathVariable Integer galeriaId) {
        MediaProducto media = service.galeriaContenido(galeriaId);
    ...
    private static ProductoResumenResponse toResponse(ProductoResumen producto) {
        return new ProductoResumenResponse(
                producto.productoId(), producto.nombre(), producto.precioUnitario(), producto.enlace(),
                producto.fecha(), producto.stock(), producto.marcaId(), producto.gamaId(), producto.ivaId(),
                producto.costo(), producto.habilitado(), producto.galeriaId());
    }
}
